...
def model_config_block(prefix, default_cfg):
    """渲染一组 provider/key/model 选择，返回配置 dict。"""
...
def render_result(tid):
    ...
    if tid in (5, 7):
        ...
        target = st.session_state.total_episodes
        if n >= target:
            st.markdown(
                f'<span class="qa-ok">✅ 集数完整性校验通过：检测到 {n} 集 / 目标 {target} 集</span>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<span class="qa-warn">⚠️ 集数校验：检测到 {n} 集 / 目标 {target} 集（真实模型生成时可点击重新执行补全）</span>',
                unsafe_allow_html=True,
            )
...
# 13A. 自动模式
# ==========================================
def _run_pipeline(viz_ph, from_progress=False):
    """按顺序执行尚未完成的任务（自动模式 / 手动模式“自动跑完剩余”共用）。返回是否全部成功。

    from_progress=True（手动模式“自动执行后续剩余任务”）：仅从“当前已完成的最高任务”的下一个任务
    开始往后执行，绝不回头补跑前序未完成任务（例如已手动完成到任务5，则只自动执行 6→9，不碰 1→4）。
    from_progress=False（自动模式“一键运行全流程”）：保持原行为，从头补齐所有未完成任务。
    """
    ok = True
    start = 1
    if from_progress:
        done_ids = [t for t in TASK_ORDER if task_done(t)]
        start = (max(done_ids) + 1) if done_ids else 1
    # 任务 1-7（跳过已完成；from_progress 时还跳过“当前进度之前”的任务，不回头补跑前序）
    for tid in [1, 2, 3, 4, 5, 6, 7]:
        ...
        if task_done(tid):
            continue
        if not is_ready(tid):
            st.warning(f"⚠️ 任务{tid} 的前置依赖未完成，自动执行已中止（可先手动粘贴该前置产出后再继续）。")
            ok = False
            break
        e = EMPLOYEES[TASK_MAP[tid]["owner"]]
        st.session_state.running_task = tid
        render_viz(viz_ph)
        with st.status(f"{e['emoji']} {e['name']} 正在执行 · 任务{tid}：{TASK_MAP[tid]['title']}", expanded=False) as s:
            res = run_generic_task(tid)
            if isinstance(res, str) and res.startswith("❌"):
                s.update(label=f"❌ 任务{tid} 失败：{res}", state="error")
                ok = False
            else:
                s.update(label=f"✅ 任务{tid} 完成 · {e['name']}", state="complete")
        render_viz(viz_ph)
        if not ok:
            break

    # 任务 8（分批，跳过已生成批次）
    if ok and 8 >= start and not task_done(8) and is_ready(8):
        st.session_state.running_task = 8
        render_viz(viz_ph)
        with st.status("⚖️ 犀利的短剧剧本审核员 正在依次生成分镜脚本…", expanded=True) as s:
            for (a, b) in get_batches(st.session_state.total_episodes):
                if f"{a}-{b}集" in st.session_state.outputs[8]:
                    continue
                st.write(f"🎬 生成 {a}-{b} 集分镜…")
                res = run_task8_batch(a, b)
                if isinstance(res, str) and res.startswith("❌"):
                    s.update(label=f"❌ 任务8（{a}-{b}集）失败：{res}", state="error")
                    ok = False
                    break
            if ok:
                s.update(label="✅ 任务8 完成 · 全部分镜脚本已生成", state="complete")
        render_viz(viz_ph)

    # 任务 9（编排归档）
    if ok and 9 >= start and not task_done(9) and is_ready(9):
        st.session_state.running_task = 9
        render_viz(viz_ph)
        with st.status("📋 文档助理 正在整理飞书交付文档…", expanded=False) as s:
            run_task9()
            s.update(label="✅ 任务9 完成 · 飞书文档已生成", state="complete")
...
def render_auto_mode(viz_ph):
    st.markdown("### 🤖 自动按序执行")
    st.info("点击下方按钮，5 位数字员工将依次自动完成任务 1 → 9，全程实时可视化，最终自动产出飞书交付文档。")
    auto_cols = st.columns([1, 3])
    start_auto = auto_cols[0].button("🚀 一键运行全流程", type="primary", use_container_width=True)
    auto_cols[1].caption(f"当前总集数：{st.session_state.total_episodes} 集 · 大纲与分镜均自动分批生成。")

    if start_auto:
        ok = _run_pipeline(viz_ph)
        if ok:
            st.balloons()
            st.success("🎉 全流程执行完毕！请在下方查看每位数字员工的任务产出。")
    ...
    for tid in TASK_ORDER:
        e = EMPLOYEES[TASK_MAP[tid]["owner"]]
        with st.expander(f"任务 {tid} · {TASK_MAP[tid]['title']}　|　{e['emoji']} {e['name']}　{status_badge(tid)}",
                         expanded=False):
...
# 13B. 手动模式
# ==========================================
def render_manual_mode(viz_ph):
    st.markdown("### 🙋 手动逐步启动")
    st.info(
        "按顺序逐个点击任务按钮启动对应数字员工；每个任务产出后可【修改】再进入下一步，"
        "也可【跳过本任务】直接粘贴本地剧本文本后保存，再继续下一步；"
        "也可随时点击下方按钮，让产品【自动跑完后续剩余任务】。"
    )

    # —— 自动执行后续剩余任务（从当前进度起一键跑完）——
    done_cnt = sum(1 for t in TASK_ORDER if task_done(t))
    rc = st.columns([1.4, 2.6])
    run_rest = rc[0].button(
        "🚀 自动执行后续剩余任务", type="primary",
        disabled=(done_cnt == len(TASK_ORDER)),
        help="从当前已完成的最后一步往后，自动执行后续任务，直到全部完成；不回头补跑前序未完成的任务",
    )
    rc[1].caption(
        f"已完成 {done_cnt}/{len(TASK_ORDER)} 个任务。"
        "点击后将从“当前进度的下一个任务”开始往后自动执行（例如手动完成到任务5，则自动执行 6→9，不再触碰 1→4）。"
    )
    if run_rest:
        _run_pipeline(viz_ph, from_progress=True)
        # 自动执行后清空编辑缓冲，使下方编辑框用最新产出重新初始化（无需 rerun）
        for tid in TASK_ORDER:
    ...
    for tid in TASK_ORDER:
        ...
        with st.container(border=True):
            ...
            c2.markdown(f"<div style='text-align:right'>{status_badge(tid)}</div>", unsafe_allow_html=True)
            st.caption(t["desc"])

            ready = is_ready(tid)
            if not ready:
                missing = [f"任务{d}" for d in t["deps"] if not task_done(d)]
                st.warning(
                    f"🔒 需先完成：{'、'.join(missing)}"
            ...
            if tid == 8:
                _render_manual_task8(ready)
            else:
                # 运行按钮（调用数字员工生成）
                btn_label = "▶️ 重新执行" if task_done(tid) else "▶️ 启动该任务（调用数字员工生成）"
                if st.button(btn_label, key=f"run_{tid}", disabled=not ready,
                             type="primary" if ready and not task_done(tid) else "secondary"):
                    st.session_state.running_task = tid
                    if tid == 9:
                        run_task9()
                    else:
                        run_generic_task(tid)
                    st.session_state.running_task = None
                    st.session_state[f"refresh_{tid}"] = True
                    st.rerun()

                # 查看 / 修改 / 粘贴 本任务产出
                with st.expander("📄 查看 / ✏️ 修改 / 📋 粘贴本任务产出", expanded=task_done(tid)):
                    if tid in (5, 7) and task_done(tid):
                    ...
                    bc = st.columns([1, 1, 2])
                    if bc[0].button("💾 保存为本任务产出", key=f"save_{tid}"):
                        o[tid] = st.session_state[f"edit_{tid}"]
                        st.rerun()
                    if bc[1].button("🧹 清空", key=f"clear_{tid}"):
                        o[tid] = None
                        st.session_state[f"refresh_{tid}"] = True
                        st.rerun()
                    if task_done(tid) and isinstance(o.get(tid), str):
                        st.download_button(
                ...
                # 任务9：额外提供飞书文档渲染预览（含分镜表格）
                if tid == 9 and task_done(tid):
                    with st.expander("📑 飞书文档渲染预览（含分镜表格）", expanded=False):
                        st.markdown(o[9])


def _render_manual_task8(ready):
    """手动模式下任务 8（分镜脚本）的控制：分批生成 / 查看表格 / 修改批次 / 粘贴本地分镜跳过。"""
    o = st.session_state.outputs
    scripts = o.get(8) or {}

    # 预同步各批次编辑缓冲（在创建批次编辑框之前）
    ...
    if ready:
        ...
        for i, (a, b) in enumerate(batches):
            ...
            if bcols[i % len(bcols)].button(f"生成 {label} {done_mark}", key=f"b8_{label}"):
                st.session_state.running_task = 8
                run_task8_batch(a, b)
                st.session_state.running_task = None
                st.session_state[f"refresh_e8_{label}"] = True
                st.rerun()
        if st.button("⚡ 一键生成全部批次分镜", key="b8_all"):
            st.session_state.running_task = 8
            for (a, b) in batches:
                run_task8_batch(a, b)
    ...
    if scripts:
        with st.expander("✏️ 修改已生成的分镜批次（CSV）", expanded=False):
            for label in list(scripts.keys()):
                ...
                if st.button(f"💾 保存批次 {label}", key=f"save8_{label}"):
                    o[8][label] = st.session_state[f"e8_{label}"]
                    st.rerun()
    ...
    with st.expander("📋 跳过生成 · 直接粘贴本地分镜（新建批次）", expanded=not scripts):
        ...
        if st.button("📌 保存为分镜批次", key="paste8_save"):
    ...
    for i, (k, e) in enumerate(EMPLOYEES.items()):
        with emp_cols[i]:
            with st.expander(f"{e['emoji']} {e['name']}", expanded=False):
                ...
                mem = st.session_state.memory.get(k, [])
                if mem:
                    for m in mem:
                        st.markdown(f"- {m}")
                else:
...
def main():
    ...
    # 可视化占位符：办公室全景 + 数字员工卡通形象（自动模式下实时刷新）
    viz_ph = st.empty()
    render_viz(viz_ph)
    ...
    if run_mode.startswith("①"):
        render_auto_mode(viz_ph)
    else:
        render_manual_mode(viz_ph)

    render_profiles()

    st.divider()
    st.caption(
        "© AI Agent 短剧剧本工作室 - zhangzhou.01@bytedance.com "
    )

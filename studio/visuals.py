...
from .state import task_done, is_ready
...
# 终极心跳保活（沿用原工具，防止断网/待机假死导致数据丢失）
_HEARTBEAT_HTML = """
    <script>
    const workerCode = `setInterval(() => { postMessage('ping'); }, 25000);`;
    ...
    setInterval(() => {
        ...
        if (currentTime - lastTime > 10000) {
            try {
                ...
                setTimeout(() => { window.parent.dispatchEvent(new Event('online')); }, 1000);
            } catch (err) { console.error("Reconnect trigger blocked."); }
        }
...
def node_status(tid):
    if st.session_state.running_task == tid:
        return "run"
    if task_done(tid):
        return "done"
    if is_ready(tid):
...
def render_viz(ph):
    with ph.container():
        ...
        st.markdown(office_scene_html(), unsafe_allow_html=True)
        st.markdown(
